...
# =============================================================================
# Telegram Magic Link (Bot-generated)
# =============================================================================

class TelegramMagicLinkRequest(BaseModel):
    """Request to verify telegram magic link token."""
    token: str


@router.post("/telegram-magic-link")
async def verify_telegram_magic_link(
    request: TelegramMagicLinkRequest = Body(...),
    db: AsyncSession = Depends(get_db)
):
    """
    Verify Telegram magic link token and return JWT tokens.
    
    This token is generated by the bot when user sends /start login
    and stored in Redis.
    """
    token = request.token
    ...
    token_data = _redis_client.get(token_key)
    ...
    if not telegram_id:
        raise HTTPException(
            status_code=400,
            detail="Token missing telegram_id"
        )
    
    # Delete token (one-time use)
    _redis_client.delete(token_key)
    ...
    logger.info(f"Telegram magic link login: telegram_id={telegram_id}")
